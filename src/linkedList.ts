import { LinkedListResult, MaybeLinkedListNode, TreeNode } from './types';

export const addNode = (treeNode: TreeNode, high: number, id: string): boolean => {
    let node: MaybeLinkedListNode = treeNode.list;
    if (!node) {
...
export const removeNode = (treeNode: TreeNode, high: number, id: string) => {
    let node: MaybeLinkedListNode = treeNode.list;
    let prevNode;
    while (node) {
        if (high > node.high) {
